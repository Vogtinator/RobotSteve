...
//Parse the .xml file containing the help texts
//It's an xml inside the resources file as it's easily expandable and also translatable
...
//Get the help text for a specific keyword, condition or instruction
QString SteveHelp::getHelp(QString word)
{
    ...
    if(keyword != SteveInterpreter::KEYWORD_INVALID && keyword_help.contains(keyword))
        return keyword_help[keyword];
    else if(instruction != SteveInterpreter::INSTR_INVALID && instruction_help.contains(instruction))
        return instruction_help[instruction];
    else if(condition != SteveInterpreter::COND_INVALID && condition_help.contains(condition))
        return condition_help[condition];
    else
        return QObject::trUtf8("Keine Hilfe für das Wort verfügbar.");
}

//Display a dialog with all keywords, conditions and instructions and their help text
void SteveHelp::showHelp()
{
    ...
    for(int i = 0; i < keyword_meta.keyCount(); i++)
    {
        ...
        if(t != SteveInterpreter::KEYWORD_INVALID)
    ...
    for(int i = 0; i < instruction_meta.keyCount(); i++)
    {
        ...
        if(t != SteveInterpreter::INSTR_INVALID)
            content.append(QString("<h2>%1</h2>%2").arg(interpreter->str(t)).arg(Qt::convertFromPlainText(getHelp(t))));
    }
    ...
    for(int i = 0; i < condition_meta.keyCount(); i++)
    {
        ...
        if(t != SteveInterpreter::COND_INVALID)
            content.append(QString("<h2>%1</h2>%2").arg(interpreter->str(t)).arg(Qt::convertFromPlainText(getHelp(t))));
    }
